...
===================================
Complete unique grammar with full alias support.
Every keyword sounds natural when spoken aloud.

Supported features:
  - store/into variable declaration
  - output/show/tell me/what is/display/print/reveal
  - ask/hear/question input
  - update/change/set/modify assignment
  - raise/increase/add/bump up
  - lower/decrease/subtract/drop down
  - when/if/suppose/check if conditions
  - do this/repeat/cycle/loop N times
  - go through/walk through/visit/step through every
  - keep going/stay/continue/repeat while
  - build/create/make/craft/define functions
  - use/call/trigger/run/activate function calls
  - store/get/capture result of
  - solve x where equations (linear + quadratic)
  - math block
  - precise decimals
  - PEMDAS expression parsing
  - parentheses grouping
  - unary minus and not
  - exponents, square root, percent
  - bitwise shifts
  - built-in formulas
  - note/remark comments
  - done/end/finish/close block closers
"""
...
KEYWORDS = {
    ...
    # input aliases
    "ask", "hear", "question", "then", "save", "prompt",
    "listen", "receive",
    # update aliases
    "update", "change", "set", "modify", "to",
    "replace", "reassign", "redefine", "overwrite",
    # raise aliases
    "raise", "increase", "add", "bump", "up", "by",
    "increment", "boost", "grow", "expand",
    # lower aliases
    "lower", "decrease", "subtract", "drop", "down", "from",
    "decrement", "reduce", "shrink", "trim", "cut",
    # condition aliases
    "when", "if", "suppose", "check", "otherwise", "else", "not",
    "assuming", "provided", "given", "unless",
    "elif", "elseif",
    # loop aliases
    "do", "this", "repeat", "cycle", "loop", "times",
    "iterate", "again",
    # for-each aliases
    "go", "walk", "visit", "step", "through", "every", "in",
    "each", "for", "scan", "traverse",
    # while aliases
    "keep", "going", "stay", "continue", "while",
    "until", "long",
    # function def aliases
    ...
    "invoke", "execute", "perform", "apply", "launch",
    "fire", "dispatch",
    # function result aliases
    "get", "capture", "result", "of",
    "grab", "fetch", "pull",
    # return
    "return", "give", "back", "yield",
    # math
    "divided", "plus", "minus", "times", "squared",
    "power", "percent", "square", "root", "joined",
    "multiplied", "added", "subtracted", "mod", "modulo",
    "negate", "absolute",
    # equations
    ...
    "empty", "true", "false",
    "greater", "less", "above", "below",
    "between", "within", "outside",
    # logic operators
    "and", "or", "nor", "xor",
    "both", "either", "neither",
    # math block
    ...
    "number", "string", "boolean", "integer", "decimal",
    "digit", "word",
    # block closers
    "done", "end", "finish", "close",
    "stop", "complete",
    # comment aliases
    "note", "remark", "comment", "describe",
    # formula keywords
    "circle", "area", "radius", "hypotenuse", "a", "b",
    "volume", "sphere", "the", "triangle", "rectangle",
    "cylinder", "cone", "perimeter", "circumference",
    "diagonal",
    # string operations
    "split", "upper", "length", "size", "count",
    "first", "last", "slice",
    # collection operations
    "pop", "append", "remove", "insert",
    "sort", "filter", "map", "index",
    # type conversion
    "convert", "cast", "parse", "format",
    # misc natural language
    "about", "around", "approximately", "exactly",
    # legacy support
    "let", "be", "as", "each", "taking",
    "forget", "know",
}

# ── Output keyword aliases ─────────────────────────────────────────────────────
OUTPUT_KEYWORDS = {
    "output", "show", "display", "print", "reveal",
    "say", "write", "log", "emit", "speak", "announce", "echo",
}

# ── Block closer aliases ───────────────────────────────────────────────────────
# NOTE: "otherwise" and "else" are intentionally NOT here — they are handled
# explicitly inside _parse_when_body so they don't terminate nested blocks.
BLOCK_CLOSERS = {"done", "end", "finish", "close", "stop", "complete"}

# ── Otherwise/else aliases — separate from BLOCK_CLOSERS ──────────────────────
OTHERWISE_WORDS = {"otherwise", "else", "elif", "elseif"}

# ── Condition opener aliases ───────────────────────────────────────────────────
...
# ── Function def aliases ───────────────────────────────────────────────────────
FUNCDEF_OPENERS = {"build", "create", "make", "craft", "define",
                   "function", "func", "method", "procedure", "routine",
                   "action", "task", "recipe"}

# ── Function call aliases ──────────────────────────────────────────────────────
FUNCCALL_OPENERS = {"use", "call", "trigger", "run", "activate",
                    "invoke", "execute", "perform", "apply", "launch",
                    "fire", "dispatch"}

# ── Update aliases ─────────────────────────────────────────────────────────────
UPDATE_OPENERS = {"update", "change", "set", "modify",
                  "replace", "reassign", "redefine", "overwrite"}
...
class Lexer:
    ...
    def read_string(self):
        ...
        if self.pos >= len(self.source):
            raise SyntaxError(
                f"Line {self.line}: Unclosed string — add a closing quote."
            )
        self.advance()
    ...
    def read_word(self):
        ...
        low = s.lower()
        if low == "true":
            return Token(TT_BOOL, True, self.line)
        if low == "false":
            return Token(TT_BOOL, False, self.line)
        if low in KEYWORDS:
            return Token(TT_KEYWORD, low, self.line)
...
class Parser:
    ...
    def parse_statement(self):
        ...
        # ── Store / declaration ─────────────────────────────────────────────
        if v == "store" or v == "remember" or v == "declare" or v == "assign" or v == "hold":
        ...
        # ── IMPORTANT: check "repeat while" BEFORE checking REPEAT_OPENERS
        # so "repeat while" is correctly routed to while-loop not count-loop
        ...
        # ── Legacy let/set/say/define/call support ───────────────────────────
        ...
        # ── num/text/precise/digit/word typed assignment ─────────────────────
        if v in ("num", "text", "precise", "digit", "word", "integer", "decimal",
    ...
    def parse_input(self):
        ln = self.peek().line
        self.advance()
        # collect prompt words until "then" or "save"
        prompt_words = []
    ...
    def parse_store(self):
        ...
        # store result of funcname with args into varname
        # Only enter if 'result' is followed by 'of' (lookahead)
        ...
        # store <expr> into <name> [precisely]
        val     = self.parse_expr()
        precise = False
        if self.match("into"):
            self.advance()
    ...
    def _parse_when_body(self, ln, _is_elif=False):
        """
        Parse a when/if/elif body.
        _is_elif=True means this is a recursively-chained elif — it must NOT
        call _consume_block_closer() because the single shared 'done' belongs
        to the outermost when block only.
        """
        ...
        if self.match_set(OTHERWISE_WORDS):
            ...
            if ow in ("elif", "elseif"):
                # Chained elif — recurse WITHOUT consuming a block closer
                inner_ln = self.peek().line
                inner    = self._parse_when_body(inner_ln, _is_elif=True)
                else_block = [inner]
                ...
                # plain "otherwise" / "else" plain block
                else_block = self.parse_block()
        # Only the outermost when consumes a block closer.
        # Chained elif nodes share that single closer.
        if not _is_elif:
    ...
    def parse_keepgoing(self):
        ln = self.peek().line
        self.advance()
        # consume "going" / "as long as" variants
        if self.match("going"):
            self.advance()
        if self.match("as"):
            self.advance()
            ...
            if self.match("as"):
                self.advance()
        # accept "while" or "until" as the condition introducer
        if self.peek().value in ("while", "until"):
            self.advance()
        cond = self.parse_condition()
        body = self.parse_block()
        self._consume_block_closer()
    ...
    def parse_build(self):
        ...
        name   = self.expect_ident().value
        params = []
        if self.match("using", "with", "taking", "accepting", "receiving",
                      "needing", "requiring", "having", "given"):
            self.advance()
    ...
    def parse_condition(self):
        ...
        # chain and/or but stop before "then" or block-starters
            ...
            if self.peek().value == kw_seq[0]:
                ...
                for kw in kw_seq:
                    if self.peek().value == kw:
                        ...
                        break
                if matched:
                    if op == "==" and self.match("empty", "true", "false",
                                                  "nothing", "null", "zero"):
                        q = self.advance().value
                        if q in ("nothing", "null"):
                            q = "empty"
                        elif q == "zero":
                            return CompNode(left, "==", LiteralNode(0))
                        return CompNode(left, "is_" + q, None)
                    right = self.parse_expr()
                    return CompNode(left, op, right)
        return left

    # ── Expression parser with full PEMDAS ───────────────────────────────────
    def parse_expr(self):
    ...
    def parse_primary(self):
        ...
        # Built-in formulas
        if t.value == "circle":
            return self.parse_formula_circle()
        if t.value == "hypotenuse":
            return self.parse_formula_hyp()
        if t.value == "volume":
            return self.parse_formula_volume()
        ...
        # Capture result expression
        # Only enter this branch if followed by 'result of' or 'of' — not bare 'result'
        ...
        # Literals
        if t.type == TT_NUMBER:
            self.advance(); return LiteralNode(t.value)
        if t.type == TT_STRING:
            self.advance(); return LiteralNode(t.value)
        if t.type == TT_BOOL:
            self.advance(); return LiteralNode(t.value)
        ...
        if t.type in (TT_IDENT, TT_KEYWORD):
            self.advance(); return IdentNode(t.value, t.line)
    ...
    def parse_formula_circle(self):
        ...
        if self.match("area"):
            self.advance()
        if self.match("with", "of"):
            self.advance()
        if self.match("radius"):
            self.advance()
